...
LogLevel log_level_from_str(const char *s)
{
    ...
    if (str_casecmp(s, "DEBUG") == 0) return LOG_DEBUG;
    if (str_casecmp(s, "INFO")  == 0) return LOG_INFO;
    if (str_casecmp(s, "WARN")  == 0) return LOG_WARN;
    if (str_casecmp(s, "ERROR") == 0) return LOG_ERROR;
    return LOG_INFO;
}
